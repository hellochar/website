...
class Nov04 extends MyPApplet with Savable {
  ...
  class Game {
    ...
    val gameStarted = Nov04.this.millis()
    ...
    (0 until 1000).foreach { i =>
      things += new Thing(me.pos + Vec2.random(1000, 10000), 3, 0, nouns(randi(nouns.length - 1)) + "s")
    }
    ...
    object me extends Thing(Vec2.random(10000), 10, 0, "you!") {
      override def draw() {
        noStroke()
        fill(255, 0, 0)
        ellipse(pos, 40, 40)
    ...
    def draw() {
      if(dead) {
        ...
        text("You lasted " + deadTime/1000f + " seconds! PRESS SPACE TO RESTART.", 0, height*2/3)
        return
      }
      ...
      background(255)
      for(x <- (-25 until width+50 by 25).map(x => floorToNearest(cam.modelX(x), 25));
          y <- (-25 until height+50 by 25).map(y => floorToNearest(cam.modelY(y), 25))
         ) {
        val sample = fn(x, y)
        if(onGround(x, y)) {
          val b = sample * 255
    ...
    def millis() = Nov04.this.millis() - gameStarted
